...
#define TPMIN_SZ2 0x177
#define TPMIN_SZ3 0x37
...
#if 1
  UINT8  TpmIn2[TPMIN_SZ2] = { 0x80,0x02,
  0x00,0x00,0x01,0x77,
  0x00,0x00, 0x01,0x31,
  0x40,0x00,0x00,0x0b,
  0x00,0x00,
...
0x00,0x00,0x00,0x00,0x00,0x00,0x00};
  ...
  UINT8  TpmIn3[TPMIN_SZ3] = { 0x80,0x02,            //TPM_ST_SESSIONS
0x00,0x00,0x00,0x37,      // Length 55 bytes
0x00,0x00,0x01,0x20,      // TPM_CC_EvictControl
0x40,0x00,0x00,0x01,      // TPM_RH_OWNER
0x80,0xFF,0xFF,0xFF,      // Handle Transient (Gets filled before use)
0x00,0x00,0x00,0x1d,0x40,0x00,
0x00,0x09,0x00,0x00,0x00,0x00,0x14,0x00,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x00,0x00,
0x81,0x01,0x00,0x01};       // Handle Persistent (0x81010001)
#endif
  ...
  if (SwapBytes32 (((TPM2_RESPONSE_HEADER*)TpmOut)->responseCode) != TPM_RC_SUCCESS) {
    DEBUG ((DEBUG_ERROR, "Failed to createprimary in TPM! Response code: 0x%x, Output:\n", SwapBytes32 (((TPM2_RESPONSE_HEADER*)TpmOut)->responseCode), __FUNCTION__));
  // Compute the checksum for the information we recieved.
  for (int i = 0; i < TPMOUT_SZ; i++) {
    DEBUG ((DEBUG_ERROR, "%x\n", TpmOut[i]));
  }
    ...
    DEBUG ((DEBUG_ERROR, "Createprimary response value: \n"));

    for (i = 0; i < Length; i = i + 8) {
      DEBUG ((DEBUG_ERROR," resp(%2.2d): 0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x\n", i,
        TpmOut[i + 0], TpmOut[i + 1], TpmOut[i + 2], TpmOut[i + 3],
        TpmOut[i + 4], TpmOut[i + 5], TpmOut[i + 6], TpmOut[i + 7]));
    }

    // Copy the handle for the EK generated by CreatePrimary into the evict command.
    CopyMem((void*)&TpmIn3[14], (void*)(TpmOut+sizeof(TPM2_RESPONSE_HEADER)), 4);
  ...
  if (SwapBytes32 (((TPM2_RESPONSE_HEADER*)TpmOut)->responseCode) != TPM_RC_SUCCESS) {
    DEBUG ((DEBUG_ERROR, "Failed to evict EK in TPM! Response code: 0x%x, Output:\n", SwapBytes32 (((TPM2_RESPONSE_HEADER*)TpmOut)->responseCode), __FUNCTION__));
  // Compute the checksum for the information we recieved.
  for (int i = 0; i < TPMOUT_SZ; i++) {
    DEBUG ((DEBUG_ERROR, "%x\n", TpmOut[i]));
  }
